...
\begin{enumerate}
	\item Since arrays as parameters to functions are effectively reference parameters
	(w/o explicitly writing the \&, you could just make a void function and change the array within it.
	\item Make a function that returns a dynamic pointer (an address on the heap).
	\item Declare a local pointer variable in the frame where the function will be called and then
	make a call to a function that returns the pointer (where you use the pointer variable
	also as the argument).
\end{enumerate}

\subsubsection*{Version 1 - using a call by reference parameter}
\begin{minted}
[frame=lines, linenos, fontsize=\small, obeytabs=true, tabsize=3]{c++}
//1) Returning and array of integers (using a call by reference param.)
//   - PARAMETER is a pointer called by reference
void return_int_array(int*& array)
{
	for (int i = 0; i < 3; i++)
	{
		array[i] = 1;
...
\subsubsection*{Version 2 - returning a dynamic pointer}
\begin{minted}
[frame=lines, linenos, fontsize=\small, obeytabs=true, tabsize=3]{c++}
//2) Returning and a dynamic pointer
int* return_dynamic_pointer()
{
...
{
	int* a;
	a = return_dynamic_pointer();
}
\end{minted}



\subsubsection*{Version 3 - return a local pointer that was declared before in other scope}
\begin{minted}
[frame=lines, linenos, fontsize=\small, obeytabs=true, tabsize=3]{c++}
//3) Returning and array of integers (by returning a pointer, using also argument)
int* return_int_array(int* array)
{
	for (int i = 0; i < 3; i++)
	{
		array[i] = 1;
